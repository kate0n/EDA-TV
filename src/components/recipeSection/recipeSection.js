...
const RecipeSection = () => {
  ...
  const togglRecipe = () => {
    recipeIndex < (data.recipies.length - 1) ? setRecipeIndex(recipeIndex + 1) : setRecipeIndex(0);
  }
  ...
  <Container>
    ...
    <section className='section-recipe'>
      ...
      <div className='section-recipe__header-text'>
        ...
        <span>Время приготовления: {data.recipies[recipeIndex].timeForCooking}</span>
        <span>Потребуется {data.recipies[recipeIndex].incridients.length + 1} ингредиентов</span>
      </div>
      ...
      <div className='section-recipe__card'>
        ...
        <div className='section-recipe__card__ingredients'>
          <h4>Ингридиенты</h4>
          {data.recipies[recipeIndex].incridients.slice(0,8).map((incridient, index) => (
              <span key={index}>{incridient}</span>
          ))}
          <span>И еще {data.recipies[recipeIndex].incridients.length - 8} ингредиентов </span>
        </div>
      </div>
      ...
      <div className='section-recipe__instruction'>
        ...
        <div className='section-recipe__instruction__steps'>
          <img src={data.recipies[recipeIndex].cookingMethod.img} alt='Первый этап приготовления' width='164px' height='164px' />
      ...
      <DownloadAppBlock url={data.recipies[recipeIndex].url} recipeTitle={data.recipies[recipeIndex].title}/>
    </section>
  </Container>
  )
};
